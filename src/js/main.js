...
const eraseButton = document.querySelector('.js_eraseButton');
...
function renderFavoriteList(){
  ...
  for(const fav of characterFavorite) {
    ulFavorite.innerHTML += `<li class="js_li_character colorCharacter" id="${fav._id}"><img src="${fav.imageUrl}" alt=""><p class="colorLetter">${fav.name}</p><button class="js_eraseButton">x</button></li>`;
  }
}
...
function handleErase (event) {
    const idErase = String(event.currentTarget.id);
